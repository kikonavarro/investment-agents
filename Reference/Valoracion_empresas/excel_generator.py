"""
Módulo para generar el Excel de valoración.
...
# ============================================================
# ESTILOS (colores del modelo Netflix)
# ============================================================
...
INPUT_FONT = Font(name="Calibri", size=10, color="FF0000FF")  # Azul para editables
...
def _set_header(ws, row, col_start, col_end, text):
    """Aplica estilo de header a una celda o rango."""
...
def _set_input(ws, row, col, value, is_pct=False):
    """Celda editable (fuente azul, fondo verde)."""
...
def generate_valuation_excel(ticker: str, data: dict, historical: dict,
    ...
    """
    Genera el Excel de valoración completo.
    ...
    Args:
        ...
        output_path: Path de salida (default: {ticker}/{ticker}_modelo_valoracion.xlsx)
    ...
    print(f"\n📈 Generando modelo de valoración Excel para {ticker}...")
    ...
    # Años de proyección: último año histórico + 5
    if sorted_years:
        last_hist_year = max(sorted_years)
    else:
        last_hist_year = 2024
    proj_years = list(range(last_hist_year + 1, last_hist_year + 6))
    all_years = sorted_years + proj_years

    # Número de años históricos y proyectados
    n_hist = len(sorted_years)
    n_proj = len(proj_years)
    n_total = n_hist + n_proj

    # Columnas: A=labels, B=vacía, luego una por año
    # Col C = primer año, etc.
    first_data_col = 3  # Columna C
    hist_cols = list(range(first_data_col, first_data_col + n_hist))
    proj_cols = list(range(first_data_col + n_hist, first_data_col + n_total))
...
def _build_assumptions_sheet(ws, ticker, company_name, segments, scenarios,
                              historical, sorted_years, proj_years, all_years,
                              first_data_col, hist_cols, proj_cols, all_cols):
    """Construye la hoja de Assumptions."""
    ...
    cell_scenario = ws.cell(row=2, column=7, value=1)  # G2 = selector
    ...
    # Offset row (hidden helper): row 3
    # G2 controla: 0=Base (fila offset 0), 1=Bull (offset 1), 2=Bear (offset 2)
    # Pero usamos: 1=Base, 2=Bull, 3=Bear -> offset = (G2-1)*gap
    ...
    for seg_idx, segment in enumerate(segments):
        ...
        # --- Revenue (resultado dinámico con OFFSET) ---
        ...
        # Proyecciones: usar OFFSET para seleccionar escenario
        # La fórmula OFFSET referencia la fila Base/Bull/Bear según G2
        row += 1  # Dejar espacio antes de growth rates

        # --- Revenue Growth Rate (3 escenarios) ---
        # Base Case Growth
        _set_label(ws, row, 1, "  Revenue Growth - Base")
        base_growth_row = row
        for i, year in enumerate(proj_years):
            col = proj_cols[i]
            growth = base_sc[f"revenue_growth_y{i+1}"]
            _set_input(ws, row, col, growth, is_pct=True)
        row += 1
        ...
        for i, year in enumerate(proj_years):
            col = proj_cols[i]
            growth = bull_sc[f"revenue_growth_y{i+1}"]
            _set_input(ws, row, col, growth, is_pct=True)
        row += 1
        ...
        for i, year in enumerate(proj_years):
            col = proj_cols[i]
            growth = bear_sc[f"revenue_growth_y{i+1}"]
            _set_input(ws, row, col, growth, is_pct=True)
        row += 1

        # --- Selected Growth (OFFSET based on scenario) ---
        _set_label(ws, row, 1, "  Revenue Growth (Selected)", bold=True)
        selected_growth_row = row
        for i, year in enumerate(proj_years):
            col = proj_cols[i]
            c_letter = _col(col)
            # OFFSET: si G2=1 -> base_growth_row, G2=2 -> bull, G2=3 -> bear
        ...
        for i, year in enumerate(proj_years):
            ...
            # Revenue = Revenue anterior * (1 + growth)
            formula = f'={prev_col_letter}{revenue_row if i == 0 else rev_proj_row}*(1+{c_letter}{selected_growth_row})'
            ws.cell(row=row, column=col, value=formula).number_format = NUM_FORMAT_M
        row += 2

    # ---- MARGEN / OPEX ASSUMPTIONS ----
    ...
    for col in proj_cols:
        ...
        ws.cell(row=row, column=col,
                value=f'=OFFSET({c}{gm_base_row},$G$2-1,0)').number_format = PCT_FORMAT
    row += 2
    ...
    for col in proj_cols:
        c = _col(col)
        ws.cell(row=row, column=col,
                value=f'=OFFSET({c}{sga_base_row},$G$2-1,0)').number_format = PCT_FORMAT
    row += 2
    ...
    for col in proj_cols:
        c = _col(col)
        ws.cell(row=row, column=col,
                value=f'=OFFSET({c}{rd_base_row},$G$2-1,0)').number_format = PCT_FORMAT
    row += 2
...
def _build_financial_statements_sheet(ws, ticker, company_name, historical,
                                        sorted_years, first_data_col, hist_cols):
    """Construye la hoja de Financial Statements con datos históricos."""

    _set_header(ws, 1, 1, max(hist_cols) if hist_cols else 5,
                f"{company_name} ({ticker}) - Historical Financial Statements")
    ...
    # ---- INCOME STATEMENT ----
    row = 5
    _set_header(ws, row, 1, max(hist_cols) if hist_cols else 5, "Income Statement")
    row += 1
    ...
    # ---- BALANCE SHEET ----
    _set_header(ws, row, 1, max(hist_cols) if hist_cols else 5, "Balance Sheet")
    row += 1
    ...
    # ---- CASH FLOW STATEMENT ----
    _set_header(ws, row, 1, max(hist_cols) if hist_cols else 5, "Cash Flow Statement")
    row += 1
...
def _build_model_sheet(ws, ticker, company_name, segments, scenarios,
                       historical, sorted_years, proj_years, all_years,
                       first_data_col, hist_cols, proj_cols, all_cols, data):
    """Construye la hoja Model con proyecciones completas."""
    ...
    segment_rev_rows = []  # Para sumar total revenue
    ...
    for seg_idx, segment in enumerate(segments):
        ...
        # Projected: referencia a Assumptions
        # Encontrar la fila de revenue proyectado en Assumptions para este segmento
        # Cada segmento tiene: header, revenue, base_growth, bull_growth, bear_growth, selected_growth, rev_proj, blank
        # Offset desde fila 6: seg_idx * 8 filas por segmento
        assumptions_rev_proj_row = 6 + seg_idx * 8 + 6  # row del Revenue Projected en Assumptions
    ...
    # Find Gross Margin Selected row in Assumptions
    n_seg = len(segments)
    gm_selected_row_assumptions = 6 + n_seg * 8 + 4  # After segments + Operating header + GM rows
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f"={c}{pl_rev_row}+{c}{cogs_row}"
        ws.cell(row=row, column=col_idx, value=formula).number_format = NUM_FORMAT_M
    row += 1
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f'=IFERROR({c}{gp_row}/{c}{pl_rev_row},"-")'
        ws.cell(row=row, column=col_idx, value=formula).number_format = PCT_FORMAT
    row += 2
    ...
    sga_selected_row_assumptions = gm_selected_row_assumptions + 6  # 4 SGA rows + blank + selected
    for i, year in enumerate(proj_years):
        col = proj_cols[i]
        c = _col(col)
    ...
    da_row_assumptions = rd_selected_row_assumptions + 2  # D&A % row
    for i, year in enumerate(proj_years):
        col = proj_cols[i]
        c = _col(col)
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f"={c}{gp_row}+{c}{sga_row}+{c}{rd_row}"
        ws.cell(row=row, column=col_idx, value=formula).number_format = NUM_FORMAT_M
    row += 1
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f'=IFERROR({c}{ebitda_row}/{c}{pl_rev_row},"-")'
        ws.cell(row=row, column=col_idx, value=formula).number_format = PCT_FORMAT
    row += 1
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f"={c}{ebitda_row}+{c}{da_row}"
        ws.cell(row=row, column=col_idx, value=formula).number_format = NUM_FORMAT_M
    row += 1
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f'=IFERROR({c}{ebit_row}/{c}{pl_rev_row},"-")'
        ws.cell(row=row, column=col_idx, value=formula).number_format = PCT_FORMAT
    row += 2
    ...
    for i, year in enumerate(sorted_years):
        ...
        _set_value(ws, row, col, val / 1e6 if abs(val) > 1e3 else val, NUM_FORMAT_M)
    # Keep last historical value for projections
    if sorted_years:
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f"={c}{ebit_row}+{c}{interest_row}"
        ws.cell(row=row, column=col_idx, value=formula).number_format = NUM_FORMAT_M
    row += 1
    ...
    taxes_row = row
    tax_rate_assumptions = da_row_assumptions + 2  # Tax Rate row
    for i, year in enumerate(sorted_years):
        col = hist_cols[i]
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f"={c}{ebt_row}+{c}{taxes_row}"
        ws.cell(row=row, column=col_idx, value=formula).number_format = NUM_FORMAT_M
    row += 1
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f'=IFERROR({c}{ni_row}/{c}{pl_rev_row},"-")'
        ws.cell(row=row, column=col_idx, value=formula).number_format = PCT_FORMAT
    row += 3
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f"={c}{cf_ebitda_row}+{c}{cf_taxes_row}+{c}{cf_wc_row}"
        ws.cell(row=row, column=col_idx, value=formula).number_format = NUM_FORMAT_M
    row += 2
    ...
    capex_row = row
    capex_row_assumptions = da_row_assumptions + 1  # CapEx % row
    for i, year in enumerate(sorted_years):
        col = hist_cols[i]
    ...
    for col_idx in all_cols:
        c = _col(col_idx)
        formula = f"={c}{ocf_row}+{c}{capex_row}"
        ws.cell(row=row, column=col_idx, value=formula).number_format = NUM_FORMAT_M
    row += 1

    # Store important row references for Valuation sheet
    ws.cell(row=1, column=1).comment = None

    # We'll store references in named cells for the Valuation sheet
    # Use a helper row at bottom to store key row numbers
    ref_row = 200
...
def _build_valuation_sheet(ws, ticker, company_name, scenarios, data,
                           proj_years, proj_cols, first_data_col, n_hist):
    """Construye la hoja de Valuation con DCF y tabla de sensibilidad."""
    ...
    # ---- DCF INPUTS from Model ----
    row = 7
    _set_header(ws, row, 1, last_proj_col, "DCF Inputs (from Model)")
    row += 1

    # Read row references from Model sheet (we know the structure)
    # These are hardcoded based on the Model sheet construction
    # We need to reference Model sheet columns for projected years

    # The Model sheet has all_cols starting at first_data_col
    # Projected columns in Model = first_data_col + n_hist + i
    model_proj_col_offset = n_hist  # offset from first_data_col
    ...
    for i in range(n_proj):
        ...
        formula = f"=Model!{model_col}$200"  # We'll fix this - use direct reference
        # Actually, let's compute from Model sheet dynamically
        # We stored refs at row 200. ebitda_row is at Model!B200
        # But we can't use variable refs in formulas. Let's use INDIRECT or just direct row
        # Better approach: compute row numbers and use them directly
        pass
    row += 1

    # Since we can't dynamically reference rows, let's use a different approach
    # We'll compute the actual row numbers based on the Model sheet structure
    # and embed them directly in formulas

    # Reset to build proper DCF
    row = 8

    # EBITDA - reference Model EBITDA row
    # The EBITDA row in Model depends on number of segments.
    # For a generic approach, we'll use INDIRECT with the stored references
    _set_label(ws, row, 1, "EBITDA ($M)")
    ebitda_dcf_row = row
    for i in range(n_proj):
        model_col = _col(first_data_col + n_hist + i)
        val_col = proj_cols[i]
        # Use INDIRECT to reference the row stored in Model!B200
    ...
    wacc_row = row
    # Find WACC Selected row in Assumptions
    n_seg = len(data["segments"])
    # After segments (n_seg * 8 rows) + Operating Assumptions (GM 4+blank + SGA 4+blank + RD 4+blank + DA + CapEx + Tax + blank)
    # + Valuation header + WACC Base,Bull,Bear,Selected
    wacc_selected_assumptions = 6 + n_seg * 8 + 1 + 5 + 1 + 5 + 1 + 5 + 1 + 1 + 1 + 1 + 1 + 1 + 4
    # Simplified: just use a cell reference
    c = _col(first_data_col)
    ws.cell(row=row, column=first_data_col, value=f"=Assumptions!{c}{wacc_selected_assumptions}").number_format = PCT_FORMAT
    row += 1

    # Terminal Value Multiple reference
    _set_label(ws, row, 1, "Terminal Value Multiple")
    tv_mult_row = row
    tv_selected_assumptions = wacc_selected_assumptions + 6  # 4 TV rows after WACC
    ws.cell(row=row, column=first_data_col,
    ...
    formula = f"={last_proj}{ufcf_dcf_row}*{tv_cell}"
    ws.cell(row=row, column=max(proj_cols), value=formula).number_format = NUM_FORMAT_M
    row += 1

    # PV of Terminal Value
    _set_label(ws, row, 1, "PV of Terminal Value ($M)")
    pv_tv_row = row
    formula = f"={last_proj}{tv_row}*{last_proj}{df_row}"
    ws.cell(row=row, column=max(proj_cols), value=formula).number_format = NUM_FORMAT_M
    row += 2
    ...
    ev_row = row
    c = _col(first_data_col)
    formula = f"={c}{sum_pv_row}+{c}{pv_tv_summary_row}"
    ws.cell(row=row, column=first_data_col, value=formula).number_format = NUM_FORMAT_M
    row += 2
    ...
    try:
        ...
        if bs is not None and not bs.empty:
            ...
            import numpy as np
            cash_val = bs.loc["Cash And Cash Equivalents", latest_col] if "Cash And Cash Equivalents" in bs.index else 0
            debt_val = bs.loc["Total Debt", latest_col] if "Total Debt" in bs.index else 0
            if cash_val is not None and not np.isnan(cash_val):
                cash = float(cash_val)
            if debt_val is not None and not np.isnan(debt_val):
                debt = float(debt_val)
            net_debt = debt - cash
    except Exception:
        pass
    ...
    equity_row = row
    c = _col(first_data_col)
    formula = f"={c}{ev_row}+{c}{net_debt_row}"
    ws.cell(row=row, column=first_data_col, value=formula).number_format = NUM_FORMAT_M
    row += 2
    ...
    evps_row = row
    c = _col(first_data_col)
    formula = f"={c}{equity_row}/{c}{shares_row}"
    cell = ws.cell(row=row, column=first_data_col, value=formula)
    ...
    c = _col(first_data_col)
    formula = f"={c}{evps_row}/{current_price}-1"
    cell = ws.cell(row=row, column=first_data_col, value=formula)
    cell.number_format = PCT_FORMAT
    ...
    # ============ SENSITIVITY TABLE ============
    _set_header(ws, row, 1, first_data_col + 8, "Sensitivity Analysis: WACC vs Terminal Value Multiple")
    row += 1

    # Column headers = TV Multiples
    ...
    for i, wacc_val in enumerate(wacc_range):
        ...
        for j, tv_val in enumerate(tv_range):
            ...
            # Formula: recalculate equity value per share with different WACC and TV
            # PV_sum = sum of UFCF_i / (1+wacc)^i
            # TV = UFCF_last * tv_multiple
            # PV_TV = TV / (1+wacc)^n
            # EV = PV_sum + PV_TV
            # Equity = (EV + net_debt) / shares

            wacc_cell = f"$A${sens_row}"
            tv_cell = f"{_col(col)}${sens_header_row - 1}"  # Will need fixing

            # Build formula using UFCF references from DCF section
            # For simplicity, build a self-contained formula
            parts = []
            ...
            tv_mult_val = tv_val
            tv_formula = f"{last_ufcf}*{tv_mult_val}/(1+$A${sens_row})^{n_proj}"
...
if __name__ == "__main__":
    ...
    print("Uso: from excel_generator import generate_valuation_excel")
